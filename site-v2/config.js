...
window.MB_ALL_PUBLIC_CURRENT = Object.freeze({
  ...
  targetIso: "2026-08-22",
  dataLock: "21/08/2026",
  dataLockIso: "2026-08-21",
...
(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      #mb-all-open-today{background:linear-gradient(135deg,#7f1d1d 0%,#b91c1c 52%,#dc2626 100%);color:#fff;padding:28px 0 30px;border-bottom:5px solid #fbbf24;box-shadow:0 16px 40px rgba(127,29,29,.24)}
      #mb-all-open-today .mball-wrap{width:min(1120px,calc(100% - 32px));margin:0 auto}
      ...
      #mb-all-open-today .mball-badge{background:#fff;color:#991b1b;border:2px solid #fde68a;border-radius:999px;padding:10px 16px;font-size:13px;font-weight:900;white-space:nowrap}
      ...
      #mb-all-open-today .mball-support{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin:0 0 18px}
      ...
      #mb-all-open-today .mball-support strong{display:block;color:#fde68a;margin-bottom:2px}
      ...
      #mb-all-open-today .mball-methods{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0;border-top:1px solid #e5e7eb}
      ...
      #mb-all-open-today .mball-method b{color:#111827}
      #mb-all-open-today .mball-method span{text-align:right;font-weight:900;color:#b91c1c}
      ...
      @media(max-width:680px){#mb-all-open-today{padding:22px 0 24px}#mb-all-open-today .mball-wrap{width:min(100% - 20px,1120px)}#mb-all-open-today .mball-picks{grid-template-columns:1fr 1fr;gap:10px}#mb-all-open-today .mball-pick{min-height:124px;border-radius:18px}#mb-all-open-today .mball-support{grid-template-columns:1fr}#mb-all-open-today .mball-methods{grid-template-columns:1fr}#mb-all-open-today .mball-method:nth-child(odd){border-right:0}#mb-all-open-today .mball-badge{white-space:normal}}
    `;
  ...
  function installCard() {
    ...
    let section = document.getElementById("mb-all-open-today");
    if (!section) {
        ...
        <div class="mball-wrap">
          <div class="mball-top">
            <div>
              ...
              <p class="mball-meta">Data Lock: <strong>${current.dataLock}</strong> · Freeze: <strong>${current.frozenAt}</strong><br>Hệ chốt: <strong>V5.0 HOT/COLD · KEP 5/5</strong></p>
            </div>
    ...
    document.querySelectorAll("[data-open-checkout], .buy-simple, .mobile-cta, #checkout").forEach((node) => {
      if (!node.classList.contains("mball-public-hidden")) node.classList.add("mball-public-hidden");
    });
    ...
    document.querySelectorAll("h1,h2,h3,p,span,button,strong").forEach((node) => {
      const text = String(node.textContent || "").trim();
      if (text === "MỞ GỢI Ý" || text === "Nhận báo cáo đầy đủ" || text === "Hiện thông tin chuyển khoản") {
        node.classList.add("mball-public-hidden");
      }
      if (text === "Phương pháp công khai hôm nay") node.textContent = "Phương pháp công khai theo dữ liệu T−1";
    });
  }

  const start = () => {
    installCard();
    setTimeout(installCard, 300);
