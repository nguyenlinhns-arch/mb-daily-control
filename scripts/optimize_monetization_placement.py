...
def remove_legacy_top_finance(text: str) -> tuple[str, bool]:
    before = text
    while 'id="lm-finance-top"' in text:
        updated, removed = remove_section(text, 'id="lm-finance-top"')
        if not removed:
            break
        text = updated
    text = re.sub(r'<style\b[^>]*id="lm-finance-top-style"[^>]*>.*?</style>', '', text, flags=re.I | re.S)
    text = text.replace('data-static-finance-banner="true"', '')
    return text, text != before
...
def apply(root: Path) -> dict[str, object]:
    ...
    text, finance_top_removed = remove_legacy_top_finance(text)
    ...
    if 'id="lm-finance-top"' in text or 'data-static-finance-banner="true"' in text:
        raise ValueError('Legacy top finance banner remains')
    ...
    return {
        'status': 'PASS',
        'changed': any((ads_removed, finance_top_removed, seo_changed, affiliate_changed)),
        'placement': 'accesstrade_after_statistics_runtime_gate',
        'static_seo_links': SEO_LINKS_MARKER in text,
        'affiliate_evergreen': True,
        'vpbank_static_top': False,
...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        page.write_text(
            '<html><head><style id="lm-final-monetization-style">x</style><style id="lm-finance-top-style">y</style></head><body>'
            '<section id="lm-finance-top" data-static-finance-banner="true"><a>VPBank top</a></section>'
        ...
        assert result['status'] == 'PASS' and result['changed'] and result['vpbank_static_top'] is False
        assert 'effectivecpmnetwork.com' not in text and 'highperformanceformat.com' not in text and 'lm-adsterra' not in text
        assert 'lm-finance-top' not in text and 'data-static-finance-banner' not in text
