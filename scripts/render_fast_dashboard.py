#!/usr/bin/env python3
"""Render a zero-dependency static dashboard from data/current.json.

The generated page contains the complete current A1/X2/X3/ROLL7 review, the
confirmed cumulative P/L and the next-draw capital.  It performs no runtime
fetch, loads no external CSS/JS and embeds no Google iframe, so GitHub Pages can
paint immediately even on a slow mobile connection.
"""
...
def visual_tone(value: Any) -> str:
    ...
    if "PASS" in text or ("ĐẠT" in text and "KHÔNG" not in text and "GẦN" not in text):
        return "pass"
    if "NEAR" in text or "GẦN" in text:
        return "near"
    if "EMPTY" in text or "TRỐNG" in text or "NOT_APPLICABLE" in text or "KHÔNG KÍCH HOẠT" in text:
        return "idle"
    return "fail"
...
def group_by_id(doc: dict[str, Any], wanted: str) -> dict[str, Any]:
    for group in doc.get("groups") or []:
        if str(group.get("id") or "").upper() == wanted.upper():
            return group
    return {}
...
def current_methods(doc: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for key in ("a1", "x2", "x3", "roll7"):
        ordered.append(found.get(key) or synthesize_method(doc, key))
    return ordered
...
def number_tile(number: dict[str, Any], method: dict[str, Any]) -> str:
    ...
    caption = f"{points} điểm" if points else "0 điểm · theo dõi"
...
def method_card(method: dict[str, Any], doc: dict[str, Any]) -> str:
    ...
    symbols = {"a1": "A1", "x2": "X2", "x3": "X3", "roll7": "R7", "other": "MB"}
    numbers = method.get("numbers") or []
    tiles = "".join(number_tile(item, method) for item in numbers)
    if not tiles:
        tiles = '<div class="empty">Không có mã được cấp vốn</div>'
    ...
    <article class="method-card {tone}">
      ...
      <footer><span>Điểm/số <b>{esc(method.get('points_per_code') or 0)}</b></span><span>Số mã <b>{esc(method.get('code_count') or 0)}</b></span><span>Vốn <b>{vnd(method.get('capital_vnd') or 0)}</b></span></footer>
    </article>"""
...
def render(doc: dict[str, Any]) -> str:
    ...
    total_capital = portfolio.get("capital_vnd") or pnl.get("today_pending_capital_vnd") or 0
