...
def calculate_settlement(doc: dict[str, Any], order: dict[str, Any], draw_date: date, codes: list[str]) -> dict[str, Any]:
    ...
    lo_points = as_int(lo.get('points_per_code'), as_int(stake.get('lo_points_per_code'), 50))
    ...
    lo_capital = len(numbers) * lo_points * lo_cost_pp
    lo_payout = sum(hits.values()) * lo_points * lo_pay_pp
    ...
    if not raw_pairs:
        ...
        raw_pairs = [f'{a}-{b}' for a, b in combinations(dedupe(base_numbers), 2)]
    ...
    return {'date': draw_date.isoformat(), 'result_sha256': json_hash(codes), 'result_codes': codes, 'lo': {'numbers': numbers, 'points_per_code': lo_points, 'hits': hits, 'hits_total': sum(hits.values()), 'capital_vnd': lo_capital, 'payout_vnd': lo_payout, 'pnl_vnd': lo_pnl}, 'xien2': {'pairs': pairs, 'points_per_pair': xien_points, 'winning_pairs': winning_pairs, 'wins': len(winning_pairs), 'losses': len(pairs) - len(winning_pairs), 'capital_per_pair_vnd': pair_capital, 'gross_return_per_winning_pair_vnd': pair_return, 'capital_vnd': xien_capital, 'payout_vnd': xien_payout, 'pnl_vnd': xien_pnl}, 'total_capital_vnd': lo_capital + xien_capital, 'total_payout_vnd': lo_payout + xien_payout, 'total_pnl_vnd': lo_pnl + xien_pnl}
...
def update_pnl_summary(doc: dict[str, Any], record: dict[str, Any], delta_lo: int, delta_xien: int) -> None:
    ...
    lo = record['lo']
    xien = record['xien2']
    q.update({'confirmed_through': record['date'], 'yesterday_date': record['date'], 'yesterday_pnl_vnd': record['total_pnl_vnd'], 'yesterday_hits': lo['hits_total'], 'yesterday_stake': f"{len(lo['numbers'])} mã ×{lo['points_per_code']} + {len(xien['pairs'])} cặp Xiên ×{xien['points_per_pair']}", 'today_pending_capital_vnd': 0, 'today_pending_order': f"Đã chốt P/L ngày: {signed_vnd(record['total_pnl_vnd'])}", 'today_included': True})
...
def full_settlement_display(doc: dict[str, Any], order: dict[str, Any], record: dict[str, Any]) -> None:
    ...
    order_lo.update({'numbers': numbers, 'points_per_code': lo['points_per_code'], 'hits': lo['hits'], 'code_count': len(numbers), 'capital_vnd': lo['capital_vnd'], 'payout_vnd': lo['payout_vnd'], 'pnl_vnd': lo['pnl_vnd']})
    order_xien = order.setdefault('xien2', {})
    order_xien.update({'pairs': pairs, 'points_per_pair': xien['points_per_pair'], 'winning_pairs': xien['winning_pairs'], 'wins': xien['wins'], 'losses': xien['losses'], 'capital_vnd': xien['capital_vnd'], 'payout_vnd': xien['payout_vnd'], 'pnl_vnd': xien['pnl_vnd']})
    order.update({'status': 'REAL_SETTLED_WIN' if total_pnl > 0 else 'REAL_SETTLED_LOSS' if total_pnl < 0 else 'REAL_SETTLED_EVEN', 'settled_at': settled_at, 'pnl_included': True, 'total_capital_vnd': record['total_capital_vnd'], 'total_payout_vnd': record['total_payout_vnd'], 'total_pnl_vnd': total_pnl, 'note': f"Lô {lo['hits_total']} nháy; Xiên thắng {xien['wins']}/{len(pairs)} cặp."})
    doc['settlement'] = {'date': record['date'], 'result_codes': record['result_codes'], 'result_sha256': record['result_sha256'], 'lo_hits_total': lo['hits_total'], 'xien_wins': xien['wins'], 'lo_pnl_vnd': lo['pnl_vnd'], 'xien_pnl_vnd': xien['pnl_vnd'], 'total_pnl_vnd': total_pnl}
    doc.pop('pending_order', None)
    doc['portfolio'] = {'decision': 'SETTLED_ACTUAL_ORDER', 'tier': 'KẾT QUẢ ĐÃ KHÓA', 'selection': '-'.join(numbers) if numbers else '|'.join(pairs), 'title': f"KẾT QUẢ {datetime.strptime(record['date'], '%Y-%m-%d').strftime('%d/%m')}: {signed_vnd(total_pnl)}", 'points': lo['points_per_code'] * len(numbers) + xien['points_per_pair'] * len(pairs), 'capital_vnd': record['total_capital_vnd'], 'payout_vnd': record['total_payout_vnd'], 'pnl_vnd': total_pnl, 'reason': order['note'], 'pnl_status': 'SETTLED'}
    methods: list[dict[str, Any]] = []
    if numbers:
        methods.append({'id': 'USER_LO', 'label': 'Lô thực chiến', 'method': f"{len(numbers)} số ×{lo['points_per_code']} điểm", 'status': order['status'], 'points_per_code': lo['points_per_code'], 'code_count': len(numbers), 'capital_vnd': lo['capital_vnd'], 'numbers': [{'code': number, 'points': lo['points_per_code'], 'capital_vnd': lo['points_per_code'] * as_int((doc.get('stake_rule') or {}).get('lo_cost_per_point_vnd'), 23000), 'role': f"{lo['hits'][number]} nháy"} for number in numbers]})
    if pairs:
        methods.append({'id': 'USER_XIEN2', 'label': 'Xiên 2 thực chiến', 'method': f"{len(pairs)} cặp ×{xien['points_per_pair']} điểm", 'status': 'SETTLED_WIN' if xien['wins'] else 'SETTLED_LOSS', 'points_per_code': xien['points_per_pair'], 'code_count': len(pairs), 'capital_vnd': xien['capital_vnd'], 'numbers': [{'code': pair, 'points': xien['points_per_pair'], 'capital_vnd': as_int((doc.get('stake_rule') or {}).get('xien2_capital_per_pair_vnd'), 100000), 'role': 'Trúng' if pair in xien['winning_pairs'] else 'Trượt'} for pair in pairs]})
    doc['top_signals'] = {'title': 'KẾT QUẢ THỰC CHIẾN ĐÃ CHỐT', 'subtitle': f"Lô {signed_vnd(lo['pnl_vnd'])} · Xiên 2 {signed_vnd(xien['pnl_vnd'])} · Tổng {signed_vnd(total_pnl)}", 'total_methods': len(methods), 'total_numbers': f'{len(numbers)} số + {len(pairs)} cặp', 'total_points': f"{lo['points_per_code'] * len(numbers)} lô + {xien['points_per_pair'] * len(pairs)} xiên", 'total_capital_vnd': record['total_capital_vnd'], 'note': order['note'], 'methods': methods}
    doc['top_signal_policy'] = {'mode': 'SETTLED_ACTUAL_ORDER', 'system_signals_preserved_in_groups': True, 'pnl_rule': 'Đã cộng kết quả đúng một lần vào sổ xác nhận.'}
