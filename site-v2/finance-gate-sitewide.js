(() => {
  ...
  const SESSION_KEY = "lm_finance_sitewide_banner_v1";
  const LAST_SHOWN_KEY = "lm_finance_sitewide_last_shown_v1";
  ...
  const emit = (event, extra = {}) => {
    ...
    window.dataLayer.push({
      ...
      placement: "early_finance_banner_sitewide",
      ...extra
    });
  };
  ...
  const lget = key => { try { return localStorage.getItem(key); } catch (_) { return null; } };
  ...
  function recentlyShown() {
    const raw = Number(lget(LAST_SHOWN_KEY) || 0);
    return Number.isFinite(raw) && raw > 0 && Date.now() - raw < COOLDOWN_MS;
  }
  function depthRatio() {
    const height = Math.max(document.documentElement.scrollHeight, document.body?.scrollHeight || 0);
    if (!height) return 0;
    return Math.min(1, (scrollY + innerHeight) / height);
  }
  function eligible() {
    return pageAllowed()
      && document.visibilityState === "visible"
      && !checkoutIsOpen()
      && sget(SESSION_KEY) !== "shown"
      && !recentlyShown()
  ...
  function showOffer(trigger) {
    if (!eligible()) return;
    stopTriggers();
    sset(SESSION_KEY, "shown");
    lset(LAST_SHOWN_KEY, String(Date.now()));
    addStyle();
    ...
    offer.querySelector(".lm-finance-secondary-cta")?.addEventListener("click", () => emit("affiliate_finance_click", { trigger }));
  ...
  function installSuppression() {
    ...
    if (document.body && "MutationObserver" in window) {
      new MutationObserver(() => { if (checkoutIsOpen()) closeOffer("checkout_open", false); })
  ...
  function installOffer() {
    ...
    timer = setTimeout(() => { if (eligible()) showOffer("early_8s"); }, MIN_DELAY_MS);
    scrollHandler = () => {
      if (!eligible()) return;
