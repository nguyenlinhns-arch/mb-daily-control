#!/usr/bin/env python3
"""Authenticated Google Sheets snapshot/apply bridge for Fusion4 transactions.
...
SOURCE_PLAN_TAB = "FUSION4_Daily_Plan"
SOURCE_SETTLEMENT_TAB = "FUSION4_Daily_Settlement"
SOURCE_LOG_TAB = "FUSION4_Automation_Log"
SOURCE_PRIVATE_CONFIG_TAB = "V32_Private_Config"
PNL_LOG_TAB = "Nhật ký FUSION4"
PNL_CONFIG_TAB = "Tự động hóa FUSION4"
PNL_METHOD_TAB = "MB FUSION4"
...
def snapshot(args: argparse.Namespace) -> None:
    ...
    required_pnl = {PNL_LOG_TAB, PNL_CONFIG_TAB, PNL_METHOD_TAB}
...
def apply_method_ledger_operation(service, pnl_id: str, operation: dict) -> None:
    """Mirror the theoretical method record without touching personal ledgers."""
    record = operation["record"]
    rows = get_values(service, pnl_id, f"{quote_tab(PNL_METHOD_TAB)}!A1:P3000")
    existing = find_operation_row(rows, operation["operation_id"])
    same_record = next((
        index for index, row in enumerate(rows[1:], start=2)
        if row and parse_date(row[0]) == record["date"]
        and len(row) > 1 and str(row[1]) == record["record_type"]
    ), None)
    if existing is None and same_record is not None:
        raise BridgeError(
            f"Xung đột {record['record_type']} ngày {record['date']} "
            f"tại {PNL_METHOD_TAB}!{same_record}"
        )
    if existing is None:
        append_row(service, pnl_id, PNL_METHOD_TAB, [
            record["date"], record["record_type"], record["status"],
            record["method"], ", ".join(record["codes"]),
            json_cell(record["points_by_code"]), record["total_points"],
            record["capital_vnd"], record.get("payout_vnd"),
            record.get("pnl_vnd"), record.get("hit_units"),
            record["source_date"], record["input_hash"],
            operation["operation_id"], operation["row_hash"],
            record["created_at"],
        ])
    checked = get_values(service, pnl_id, f"{quote_tab(PNL_METHOD_TAB)}!A1:P3000")
    row_index = find_operation_row(checked, operation["operation_id"])
    if row_index is None or operation["row_hash"] not in {
        str(value) for value in checked[row_index - 1]
    }:
        raise BridgeError("Readback sổ phương pháp FUSION4 thất bại")
...
def apply_personal_operation(service, pnl_id: str, operation: dict) -> None:
    ...
    marker = f"[FUSION4:{operation['operation_id'][:12]}]"
...
def apply(args: argparse.Namespace) -> None:
    ...
    for operation in payload["operations"]:
        ...
        if kind.startswith("UPSERT_SOURCE_"):
            ...
            apply_method_ledger_operation(service, pnl_id, operation)
            append_log(
            ...
            append_log(
                ...
                payload, operation, "METHOD_LEDGER_MIRRORED_VERIFIED",
            )
    ...
    receipt = {
        "schema_version": "MB_FUSION4_SHEETS_RECEIPT_V1",
