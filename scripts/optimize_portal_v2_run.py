...
def apply(root:Path):
    ...
    return {'status':'PASS','updated_through':stats['updated_through'],'target_date':target_date,'data_lock':data_lock,'daily_recommendation_heading':True,'daily_recommendation_subtitle':True,'daily_offer_static':True,'copy_lock':True,'affiliate_restore':True,'new_pages':2,'consensus':len(v2.method_consensus(public_methods)),'stats_assets_externalized':5}
