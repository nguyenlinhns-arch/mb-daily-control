...
JS = r'''(()=>{let D,selectedNumber='';const version=document.documentElement.dataset.statsVersion||'';const statsUrl='/statistics-data.json'+(version?'?v='+encodeURIComponent(version):'');const load=()=>D?Promise.resolve(D):fetch(statsUrl,{cache:'default'}).then(r=>{if(!r.ok)throw Error('data');return r.json()}).then(x=>D=x);const emit=(e,p={})=>{window.dataLayer=window.dataLayer||[];window.dataLayer.push({event:e,page_path:location.pathname,...p})};const fmt=s=>s?s.split('-').reverse().join('/'):'Chưa có';const active=()=>document.querySelector('.tabs button.active')?.dataset.w||'60';function profile(code){const r=D.numbers.find(x=>x.code===code),w=active(),s=r?.windows?.[w],box=document.querySelector('#profile');if(!r||!s||!box)return;selectedNumber=code;box.innerHTML=`<h3>Hồ sơ số ${code}</h3><div class="kpis"><div class="kpi"><small>Ngày có mặt /${w}</small><b>${s.days_seen}</b></div><div class="kpi"><small>Tổng nháy /${w}</small><b>${s.hits}</b></div><div class="kpi"><small>Tỷ lệ ngày có mặt</small><b>${s.rate}%</b></div><div class="kpi"><small>Gan hiện tại</small><b>${r.current_gap} kỳ</b></div></div><small>Lần gần nhất: ${fmt(r.last_seen)} · Gan max: ${r.max_gap} kỳ</small>`;emit('xsmb_number_open',{number:code,window:w})}function renderWindow(w){if(!D)return;const byCode=new Map(D.numbers.map(r=>[r.code,r]));document.querySelectorAll('.num[data-number]').forEach(btn=>{const r=byCode.get(btn.dataset.number),s=r?.windows?.[w],small=btn.querySelector('small');if(r&&s&&small)small.textContent=`${s.days_seen}/${w} · ${s.hits} nháy · gan ${r.current_gap}`});const title=document.querySelector('#quick-window-title');if(title)title.textContent=`Nhìn nhanh ${w} kỳ`;const body=document.querySelector('#quick-hot-body');if(body){const hot=[...D.numbers].sort((a,b)=>(b.windows[w].days_seen-a.windows[w].days_seen)||(b.windows[w].hits-a.windows[w].hits)||a.code.localeCompare(b.code)).slice(0,10);body.innerHTML=hot.map(r=>`<tr><td><b>${r.code}</b></td><td>${r.windows[w].days_seen}/${w}</td><td>${r.windows[w].hits}</td><td>${r.current_gap}</td></tr>`).join('')}if(selectedNumber)profile(selectedNumber)}document.addEventListener('click',async e=>{const t=e.target.closest('.tabs button');if(t){await load();document.querySelectorAll('.tabs button').forEach(x=>x.classList.toggle('active',x===t));renderWindow(t.dataset.w);emit('xsmb_stats_window',{window:t.dataset.w});return}const n=e.target.closest('[data-number]');if(n){await load();profile(n.dataset.number)}const a=e.target.closest('[data-affiliate]');if(a)emit('affiliate_click',{affiliate_offer_id:a.dataset.affiliate})});document.addEventListener('submit',async e=>{if(!e.target.matches('#lookup'))return;e.preventDefault();await load();const f=new FormData(e.target),codes=[...new Set((String(f.get('numbers')||'').match(/\d{1,2}/g)||[]).map(x=>String(Number(x)).padStart(2,'0')).filter(x=>+x>=0&&+x<=99))].slice(0,20),days=+f.get('days'),rows=D.recent_history.slice(-days).reverse().map(r=>{const [d,...v]=r,c=new Map();v.forEach(x=>c.set(x,(c.get(x)||0)+1));const h=codes.filter(x=>c.has(x));return h.length?[d,h.map(x=>x+(c.get(x)>1?' ×'+c.get(x):''))]:null}).filter(Boolean),out=document.querySelector('#lookupOut');out.innerHTML=codes.length?`<p>Bộ <b>${codes.join(', ')}</b> xuất hiện ít nhất một số trong <b>${rows.length}/${Math.min(days,D.recent_history.length)}</b> kỳ đã dò.</p>${rows.length?`<div class="scroll"><table><thead><tr><th>Ngày</th><th>Số xuất hiện</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${fmt(r[0])}</td><td>${r[1].join(' · ')}</td></tr>`).join('')}</tbody></table></div>`:''}`:'<p>Hãy nhập ít nhất một số 00–99.</p>';emit('xsmb_lookup',{numbers:codes.join('-'),days,matched_days:rows.length})});})();'''
...
def shell(title: str, desc: str, path: str, data: dict[str, Any], body: str, cfg: dict[str, Any], zone: str) -> str:
    ...
    version=data['source_sha256'][:12]
    return f'''<!doctype html><html lang="vi" data-xsmb-stats="v1" data-stats-version="{version}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="index,follow,max-snippet:-1,max-image-preview:large"><title>{esc(title)}</title><meta name="description" content="{esc(desc)}"><link rel="canonical" href="{esc(url)}"><link rel="icon" href="/favicon.svg"><style>{CSS}</style><script async src="https://www.googletagmanager.com/gtag/js?id=G-R9TBYP97BC"></script><script>window.dataLayer=window.dataLayer||[];function gtag(){{dataLayer.push(arguments)}}gtag('js',new Date());gtag('config','G-R9TBYP97BC',{{allow_google_signals:false,allow_ad_personalization_signals:false}});</script><script type="application/ld+json">{schema}</script></head><body><header class="top"><div class="topin"><a class="brand" href="/">LÊ MIỀN BẮC<small>Thống kê dữ liệu XSMB</small></a><a class="buy" href="/?checkout=1">Báo cáo 4SO hôm nay</a></div><nav class="nav"><a href="/">Trang chủ</a><a href="/cho-so-mien-bac-hom-nay/">Số hôm nay</a><a href="/thong-ke-xsmb/">Thống kê XSMB</a><a href="/tan-suat-xsmb/">Tần suất</a><a href="/lo-gan-xsmb/">Lô gan</a><a href="/cap-dao-xsmb/">Cặp đảo</a><a href="/tra-cuu-xsmb/">Tra cứu</a></nav></header><main class="main"><div class="source"><b>Dữ liệu đến {dmy(data['updated_through'])}</b><span>27/27 mã mỗi ngày · {data['row_count']} kỳ · SHA-256 {version}…</span></div>{body}{affiliate_box(cfg,zone)}<section class="note"><b>Cách đọc:</b> đây là thống kê mô tả dữ liệu đã công bố. Khoảng vắng không có nghĩa một số “đến hạn” phải xuất hiện. Các trang này không công bố Top 2 canonical, Score hay thứ hạng 4SO.</section></main><footer class="footer">Lê Miền Bắc · Thống kê dữ liệu XSMB · <a href="/legal.html">Điều khoản &amp; bảo mật</a></footer><script>{JS}</script></body></html>'''


def hub(data: dict[str, Any]) -> str:
    grid=''.join(f'<button class="num" data-number="{r["code"]}"><b>{r["code"]}</b><small>{r["windows"]["60"]["days_seen"]}/60 · {r["windows"]["60"]["hits"]} nháy · gan {r["current_gap"]}</small></button>' for r in data['numbers'])
...
def lookup() -> str:
    return '''<section class="hero"><p class="eyebrow">TRA CỨU BỘ SỐ</p><h1>Dò một bộ số trong lịch sử XSMB</h1><p>Nhập ví dụ <b>05, 50, 38, 83</b>; hệ thống liệt kê những kỳ có ít nhất một số xuất hiện và số nháy tương ứng.</p></section><section class="panel"><form id="lookup" class="lookup"><input name="numbers" placeholder="05, 50, 38, 83" inputmode="numeric" required><select name="days"><option value="30">30 kỳ</option><option value="60" selected>60 kỳ</option><option value="90">90 kỳ</option><option value="100">100 kỳ</option><option value="365">365 kỳ</option></select><button>Dò lịch sử</button></form><div id="lookupOut"></div></section>'''
...
def self_test() -> None:
    ...
    assert d['windows']==[7,14,30,60,90,100,365] and '90' in d['numbers'][0]['windows'] and '90' in d['pairs'][0]['windows']
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        hub_html=(out/'thong-ke-xsmb/index.html').read_text(); freq_html=(out/'tan-suat-xsmb/index.html').read_text(); lookup_html=(out/'tra-cuu-xsmb/index.html').read_text()
        assert 'data-window-dynamic="true"' in hub_html and '90 kỳ' in hub_html and 'quick-hot-body' in hub_html
        assert '<th>90</th>' in freq_html and '<option value="90">90 kỳ</option>' in lookup_html
        assert 'data-stats-version=' in hub_html and "cache:'default'" in hub_html
